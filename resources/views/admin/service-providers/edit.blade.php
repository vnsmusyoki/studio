@extends('layouts.app')
@section('title', 'Admin | Edit Service Provider')

@section('content')
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <h6 class="fw-semibold mb-0">Edit Service Provider</h6>
        <ul class="d-flex align-items-center gap-2">
            ...
            <li>-</li>
            <li class="fw-medium">Edit Service Provider Details</li>
        </ul>
    </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form class="row gy-3 needs-validation" autocomplete="off" enctype="multipart/form-data" method="POST"
                        action="{{ route('admin.providers.update', $provider->id) }}" >
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="col-md-6">
                            ...
                            <input class="form-control @error('phone_number') is-invalid @enderror" type="text"
                                name="phone_number" value="{{ old('phone_number', $provider->phone_number) }}" required />
                            @error('phone_number')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            @if ($provider->profile_picture)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $provider->profile_picture) }}" alt="Profile Picture"
                            ...
                            <select class="form-select @error('service_category_id') is-invalid @enderror"
                                ...
                                @foreach ($serviceCategories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $provider->service_category_id == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea class="form-control" name="description" id="description" rows="5">{{ $provider->description ?? old('description') }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="status" required>
                                <option value="pending" {{ $provider->status == 'pending' ? 'selected' : '' }}>Pending
                                </option>
                                <option value="approved" {{ $provider->status == 'approved' ? 'selected' : '' }}>Approved
                                </option>
                                <option value="blocked" {{ $provider->status == 'blocked' ? 'selected' : '' }}>Blocked
                                </option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <hr>
                    @if ($provider->portfolio_images)
                    <div class="row mt-3">
                        @foreach (json_decode($provider->portfolio_images, true) as $key => $image)
                            <div class="col-md-2 mb-3 text-center">
                                <img src="{{ asset('storage/' . $image) }}" alt="Portfolio"
                                    class="img-fluid rounded mb-2" style="height: 100px; object-fit: cover;">
                                <form method="POST"
                                    action="{{ route('admin.providers.deleteImage', [$provider->id, $key]) }}">
                                    @csrf
                                    @method('DELETE')
