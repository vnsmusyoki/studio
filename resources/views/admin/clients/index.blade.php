                        ...
                        <tr>
                            <td>
                                @if($client->profile_picture)
                                    <img src="{{ asset('storage/' . $client->profile_picture) }}" alt="Profile Picture" style="height: 48px; width: 48px;border-radius: 50%;">
                                @else
                                    <img src="https://ui-avatars.com/api/?name={{ $client->name }}" alt="Profile Picture" class="rounded-circle" width="32" height="32">
                                @endif
                            </td>
                            <td>{{ $client->name }}</td>
                            <td>{{ $client->email }}</td>
                            ...
                            <td class="text-center">
                                @if ($client->status == 'approved')
                                    <span class="badge bg-success">{{ ucfirst($client->status) }}</span>
                                @elseif($client->status == 'blocked')
                                    <span class="badge bg-danger">{{ ucfirst($client->status) }}</span>
                                @else
                                    <span class="badge bg-warning text-dark">{{ ucfirst($client->status) }}</span>
                                @endif
                            </td>
                            <td class="text-center">
                                <div class="d-flex align-items-center gap-2 justify-content-center">
                                    @if($client->status == 'pending')
                                        <form action="{{ route('admin.providers.confirm', $client->id) }}" method="POST" style="display:inline-block;">
                                            @csrf
                                    ...
                                    @endif
                                    <a href="{{ route('admin.providers.edit', $client->id) }}" class="btn btn-primary btn-sm">
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.providers.destroy', $client->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
